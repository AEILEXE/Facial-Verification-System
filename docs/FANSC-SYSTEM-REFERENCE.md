...
`CLIENT-SETUP/refresh-server-connection.ps1` (`.bat` launcher) automates the fix on one client PC at a time: it computes that PC's own local subnet, ping-sweeps it, and asks each live host FANS-C's existing `/health/network/` endpoint (no new server feature or open port — already used by the installer) whether it is the FANS-C server (`"service": "fans-c"` in the JSON body), then rewrites only the `fans-barangay.local` hosts-file line if exactly one match is found. This is LAN-only (never leaves the client's own subnet), makes no Internet call, and does not weaken certificate validation for the real site — the identification probe alone skips certificate checking (it is contacting candidate IPs, not the certificate's hostname), and normal HTTPS with full validation resumes once the hosts entry is corrected and the browser opens `https://fans-barangay.local`. **Limitation:** this must be run again on **each** client PC after the server changes networks — it is not automatic/background, and it cannot repair other client machines remotely; a subnet larger than a /22 is deliberately refused rather than scanned.
