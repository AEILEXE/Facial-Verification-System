...
**Name resolution is a static, per-client hosts-file entry — there is no mDNS/DNS server.** `fans-barangay.local` only resolves because each client's `trust-local-cert.bat` run wrote `<server-LAN-IP>  fans-barangay.local` into that PC's own hosts file at setup time. If the server PC is later shut down and reconnected to a **different** Wi-Fi/network, it is issued a new DHCP address; every client's hosts file still has the *old* address, so `https://fans-barangay.local` fails on every client until each one's hosts entry is corrected — the server auto-detecting its own new LAN IP (`fans/context_processors.py::_detect_lan_ip`, used for `ALLOWED_HOSTS` and the on-screen "how to connect" banner) does **not** propagate that address to clients by itself.

`CLIENT-SETUP/refresh-server-connection.ps1` (`.bat` launcher) automates the fix on one client PC at a time: it computes that PC's own local subnet, TCP-probes port 443 on every candidate address (not ICMP ping — Windows Firewall blocks ICMP Echo by default on many machines, which would otherwise hide a fully-reachable server), and asks each live host FANS-C's existing `/health/network/` endpoint (no new server feature or open port — already used by the installer) whether it is the FANS-C server (`"service": "fans-c"` in the JSON body), then rewrites only the `fans-barangay.local` hosts-file line if exactly one match is found. This is LAN-only (never leaves the client's own subnet), makes no Internet call, and does not weaken certificate validation anywhere — the identification probe validates each candidate's certificate against `rootCA.pem` (shipped alongside the script, the same file `trust-local-cert.bat` already imported into the client's trust store) with the hostname pinned via `curl --resolve`, so a candidate must present a certificate actually issued by the FANS-C server's own CA to be accepted; an unrelated device on the LAN cannot pass the check just by listening on port 443. If 0 or 2+ machines answer as FANS-C, the script refuses to guess and fails safely rather than picking one arbitrarily. **Limitation:** this must be run again on **each** client PC after the server changes networks — it is not automatic/background, and it cannot repair other client machines remotely; a subnet larger than a /22 is deliberately refused rather than scanned.
...
| Can't reach `fans-barangay.local` | Confirm the hosts entry / DNS resolution on the client, and that Caddy is running. If the server recently moved to a different network, run `CLIENT-SETUP/refresh-server-connection.bat` on the client first (see §33) |
