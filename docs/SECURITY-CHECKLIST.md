...
**2026-09-08 documentation-accuracy correction (no behavior change):**
- **Low-quality override is dormant on the common transaction-bound verification path.** The v2.1.13 entry below describes `LOW_QUALITY_FORCES_MANUAL_REVIEW` as downgrading "any auto-verify decision" — that was accurate for the identity path that existed at the time, but the liveness-proof-bound (`LivenessTransaction`) path added since compares the *stored liveness-frame embedding*, not a freshly re-processed frame, so `apply_quality_override()` is only ever invoked with a non-`None` quality dict on `verify_submit`'s rare fallback path (no TX embedding available — e.g. a mocked model in tests). On the transaction-bound path, which is effectively all production traffic when `LIVENESS_PROOF_REQUIRED=True` (the default), this override does not currently run. Neutral-frame quality *is* computed upstream in `verify_check_liveness` (Mode B) for the audit record, but it is not presently threaded into the transaction-bound final decision. This is a pre-existing, previously-undocumented behavior gap re-confirmed during the FANSC v2.1.17 biometric-verification audit (2026-09-08) — not a new regression, and not changed in that pass. Activating the override on the transaction-bound path is a biometric decision-policy change and requires its own reviewed pass, not a documentation fix.
...
- **Low-quality capture forces MANUAL_REVIEW — fallback identity path only (see 2026-09-08 correction below).** `LOW_QUALITY_FORCES_MANUAL_REVIEW=True` (default) downgrades a `verified` decision to MANUAL_REVIEW when face quality is degraded, via `face_utils.apply_quality_override()`. Low-quality embeddings produce coincidental high similarities; a human must confirm.
