...
| Low-quality forced review | `LOW_QUALITY_FORCES_MANUAL_REVIEW=True` (default). Even when similarity ≥ 0.88, a low-quality face capture is downgraded to MANUAL_REVIEW so a human confirms. Low-quality embeddings can produce coincidental high similarities. |
