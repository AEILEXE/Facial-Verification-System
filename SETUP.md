...
| Model | How it loads |
|---|---|
| FaceNet (keras-facenet) | Bundled with the installer (~90 MB) at `<install folder>\models\keras-facenet\` (e.g. `C:\FANSC\models\keras-facenet\` — NOT `%USERPROFILE%\.keras-facenet\` or `~/.keras\`, which keras-facenet would use by default). No internet required on first run. |
...
> **First run:** The FaceNet model (~90 MB) ships pre-cached in `<install folder>\models\keras-facenet\20180402-114759\` — a machine-local location next to `fans_c.exe`, not under any particular Windows account's `%USERPROFILE%`. All startups on this machine load from that cache — no internet required — regardless of which Windows account starts FANS-C. If the cache is deleted or the server is moved to a new machine without copying that folder, internet access is needed to re-download it. FANS-C never silently substitutes a fake/random model when the real one is unavailable — registration and verification are blocked with a clear "model unavailable" message instead (see Troubleshooting below).
>
